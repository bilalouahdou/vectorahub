-- Email tracking and webhook support tables for VectraHub (Fixed Version)
...
-- Only create index if expires_at column exists
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'user_subscriptions' AND column_name = 'expires_at') THEN
...
VALUES 
    ('email_from_name', 'VectraHub'),
    ('email_from_address', 'noreply@vectrahub.online'),
    ('email_admin_address', 'admin@vectrahub.online'),
